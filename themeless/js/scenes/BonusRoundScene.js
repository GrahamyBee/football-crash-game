class BonusRoundScene extends Phaser.Scene {
    ...
    create() {
        ...
        // Create 5 clickable zones with cash amounts
        ...
        // Instructions
        this.add.text(width / 2, 80, 'SELECT A TARGET', {
            fontSize: '28px',
            fontStyle: 'bold',
            fill: '#FFFFFF',
            stroke: '#000000',
    ...
    takeShot(selectedZone) {
        ...
        // THEMELESS: Show result immediately
        if (isGoal) {
            this.handleGoal(cashAmount, targetX, targetY);
        } else {
            this.handleSave(targetX, targetY);
        }
    }
    
    handleGoal(cashAmount, targetX, targetY) {
        // THEMELESS: Show HIT text immediately at target
        const hitText = this.add.text(targetX, targetY, 'HIT!', {
            fontSize: '80px',
            fontStyle: 'bold',
            fill: '#00FF00',
            stroke: '#000000',
        ...
        // Show cash amount
        const amountText = this.add.text(targetX, targetY + 80, `£${(cashAmount / 100).toFixed(2)}`, {
            fontSize: '50px',
            fontStyle: 'bold',
            fill: '#FFD700',
            stroke: '#000000',
    ...
    handleSave(targetX, targetY) {
        // THEMELESS: Show MISS text immediately at target
        const missText = this.add.text(targetX, targetY, 'MISS!', {
            fontSize: '80px',
            fontStyle: 'bold',
            fill: '#FF0000',
            stroke: '#000000',
